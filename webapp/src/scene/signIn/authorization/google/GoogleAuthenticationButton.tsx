...
const GoogleAuthenticationButton: () => JSX.Element = () => {
    const SignUpWithGoogle = (): void => {
        const params = {
            client_id: authParams.clientId as string,
            redirect_uri: authParams.redirectUri as string,
            scope: authParams.scope as string,
            response_type: authParams.responseType as string,
            approval_prompt: authParams.approvalPrompt as string,
            access_type: authParams.accessType as string,
        };
    ...
    const buttonText = isLoginPage ? "Login with Google" : "Sign up with Google";

    return (
        <button className="gsi-material-button" onClick={SignUpWithGoogle}>
